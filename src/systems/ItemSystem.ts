...
import { loadInventoryFromDatabase, saveInventoryToDatabase } from '../utils/Database';
...
const ITEM_TEMPLATES: { [key: string]: Omit<InventoryItem, 'quantity'> } = {
    'health_crystal': {
        id: 'health_crystal',
        name: 'Health Crystal',
...
export class ItemSystem {
  ...
  private unsubscribeInventoryListener: (() => void) | null = null;
  private onInventoryUpdateCallback: (() => void) | null = null;

  constructor() {
    // Try to load player ID from localStorage
    try {
      const userDataStr = typeof window !== 'undefined' && window.localStorage ? 
        window.localStorage.getItem('niftdood-player') : null;
      if (userDataStr) {
  ...
  public setOnInventoryUpdateCallback(callback: () => void): void {
  ...
  public subscribeToInventoryUpdates(): void {
    if (!this.playerId) {
      console.warn('⚠️ ItemSystem: No player ID available, cannot subscribe to inventory updates');
      return;
    }
    
    try {
      const inventoryRef = collection(db, "players", this.playerId, "inventory");
      const q = query(inventoryRef);
      
      this.unsubscribeInventoryListener = onSnapshot(q, (snapshot) => {
        console.log('🔄 Real-time inventory update received');
        // Update local inventory with Firestore data
        this.inventoryItems = snapshot.docs.map(document => document.data() as InventoryItem);
        
        // Trigger callback if set
        if (this.onInventoryUpdateCallback) {
          this.onInventoryUpdateCallback();
        }
      }, (error) => {
        console.error('❌ Error subscribing to inventory updates:', error);
      });
      
      console.log('✅ Subscribed to real-time inventory updates');
    } catch (error) {
  ...
  public unsubscribeFromInventoryUpdates(): void {
    if (this.unsubscribeInventoryListener) {
      this.unsubscribeInventoryListener();
      this.unsubscribeInventoryListener = null;
      console.log('✅ Unsubscribed from real-time inventory updates');
    }
  }
  ...
  // Save inventory to Firestore with retry logic
  public async saveInventory(retryCount: number = 3): Promise<boolean> {
    for (let attempt = 1; attempt <= retryCount; attempt++) {
      try {
        ...
        console.log('✅ Saved inventory to Firestore (attempt', attempt, ')');
        return true;
      } catch (error) {
        console.error(`❌ Error saving inventory to Firestore (attempt ${attempt}/${retryCount}):`, error);
        if (attempt === retryCount) {
          // If all retries failed, return false
          console.error('❌ All save attempts failed, inventory not saved');
          return false;
        }
