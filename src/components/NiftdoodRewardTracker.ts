// src/components/NiftdoodRewardTracker.ts
...
export default class NiftdoodRewardTracker {
    ...
    private setupEventListeners(): void {
        ...
        if (typeof window !== 'undefined') {
            window.addEventListener('niftdoodRewardAdded', this.rewardAddedHandler);
            window.addEventListener('niftdoodLogCleared', this.logClearedHandler);
        }
    ...
    private updateDisplay(): void {
        ...
        if (stats.rewardCount > 0) {
            statsText += `Total: ${stats.totalRewards.toFixed(2)} Niftdoods\n`;
            statsText += `Rewards: ${stats.rewardCount}`;
        } else {
            statsText += `Total: 0 Niftdoods\nNo rewards yet`;
        }
    ...
    public destroy(): void {
        ...
        if (typeof window !== 'undefined') {
            window.removeEventListener('niftdoodRewardAdded', this.rewardAddedHandler);
            window.removeEventListener('niftdoodLogCleared', this.logClearedHandler);
        }
