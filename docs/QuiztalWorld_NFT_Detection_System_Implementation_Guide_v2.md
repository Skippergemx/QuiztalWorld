# Quiztal World NFT Detection System Implementation Guide (v2)

## Overview
This document provides a step-by-step guide for implementing the NFT Detection System with the "N/n" keyboard binding in Quiztal World. This system will allow players to quickly check their NFT collection and activate skill-based power-ups based on their holdings, integrating with the existing WalletWindowScene.

## System Components
1. N Key Binding ("N/n")
2. Integration with existing WalletWindowScene
3. Skill System (Temporary Power-ups)
4. UI Integration with existing UIScene

## Prerequisites
- Phaser.js game framework (already in use)
- Existing WalletWindowScene implementation
...
export default class GameScene extends Phaser.Scene {
  ...
  private async handleNFTDetection(): Promise<void> {
    ...
    // Launch the Wallet window scene which handles NFT display
...
First, add the property declaration:

```typescript

export default class UIScene extends Phaser.Scene {
    // ... existing properties ...
    private keyN!: Phaser.Input.Keyboard.Key; // <-- Add this property
    
```

Then add the key binding in the create method:
...
#### 6.1 Modify WalletWindowScene to Activate Skills
Edit `src/scenes/WalletWindowScene.ts`:

First, add the imports and properties:
...
export default class WalletWindowScene extends Phaser.Scene {
    // ... existing properties ...
    private skillActivationTimer: Phaser.Time.TimerEvent | null = null;
    
```

Then, modify the `activateNFTBenefits` method to also activate skills:

```typescript

/**
 * Activate player glow, titles, and moblin pet for NFT holders
 * Also activate skill-based power-ups
 */
private async activateNFTBenefits(): Promise<void> {
    try {
        ...
        if (gameScene && gameScene.playerManager) {
            ...
            // Activate skill-based power-ups
            await this.activateNFTRewards(gameScene);
        }
    } catch (error) {
...
// ADD THIS NEW METHOD
private async activateNFTRewards(gameScene: any): Promise<void> {
    try {
        // Get NFT data from localStorage
        const nftsStr = localStorage.getItem('quiztal-nfts');
        if (!nftsStr) {
            console.log('No NFTs found for skill activation');
            return;
        }
        
        const nfts: any[] = JSON.parse(nftsStr);
        if (nfts.length === 0) {
            console.log('No NFTs found for skill activation');
            return;
        }
        ...
        const rewards = this.getNFTRewards(nfts);
        
        if (rewards.length > 0 && gameScene.playerManager) {
            const skillSystem = gameScene.playerManager.getSkillSystem();
...
// ADD THIS NEW METHOD
private getNFTRewards(nfts: any[]): SkillEffect[] {
    ...
    for (const nft of nfts) {
        ...
        if (nft.collectionType === 'erc1155' && nft.name && nft.name.includes('Gemante')) {
...
private showNotification(message: string): void {
    ...
    this.scene.get('UIScene')?.events.emit('showNotification', { message });
}

```

### 7. Adding Notification System to UIScene

#### 7.1 Add Notification Display to UIScene
Edit `src/scenes/UIScene.ts`:
...
create() {
  try {
    ...
    // Add event listener for notifications from WalletWindowScene
    this.scene.get('WalletWindowScene')?.events.on('showNotification', this.showNotification, this);
...
// Expected: Wallet window opens and displays NFTs
...
## Conclusion
This implementation provides a complete NFT detection system with keyboard binding and skill system integration specifically tailored for Quiztal World. The system integrates with the existing WalletWindowScene and leverages existing UI components while adding new functionality for skill-based power-ups based on NFT holdings.